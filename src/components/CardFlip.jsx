    ...
    >
      ...
      >
        ...
        >
          <div className="absolute inset-0 bg-[#F5F5DC] opacity-10 group-hover:opacity-50" />
          <div className="relative text-center flex flex-col justify-center items-center h-full w-full p-4">
            <h1 className="text-xl md:text-2xl font-bold mb-4">{title2}</h1>
            <p className="text-base md:text-lg">{content2}</p>
          </div>
        </div>
      </motion.div>
    </div>
...
/*const CardFlip = ({ title, content, title2, content2, backgroundImagef, backgroundImageb }) => {
    ...
    return (
        ...
        <div className="flex flex-col gap-6 group relative shadow-lg h-64 w-64 sm:h-72 sm:w-72 md:h-130 md:w-96 lg:h-210 lg:w-96 overflow-hidden cursor-pointer z-20" onClick={handleFlip}>
        <motion.div
                className="flip-card-inner w-full h-full"                
                initial={false}
                animate={{ rotateY: isFlipped ? 180 : 360 }}
